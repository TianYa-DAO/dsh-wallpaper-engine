...
function bound(value: unknown, minimum: number, maximum: number, fallback: number): number {
...
export function readWallpaperSelection(): WallpaperSelection {
  try {
    const raw = JSON.parse(localStorage.getItem(WALLPAPER_SELECTION_STORAGE_KEY) ?? '{}') as Record<string, unknown>
    const id = normalizeId(raw.id)
    const kind: WallpaperSelectionKind = raw.kind === 'engine' ? 'engine' : (raw.kind === 'media' ? 'media' : 'preview')
    return {
...
export function emptyWallpaperSelection(): WallpaperSelection {
  return {
    active: false,
    id: '',
    title: '',
...
export function writeWallpaperSelection(selection: WallpaperSelection): void {
  try {
