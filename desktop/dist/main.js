...
function windowParkScript(input) {
	...
	return desktopPowerShellScript([
		...
		"# The playback window is hosted by whichever WE process actually runs",
		"# (wallpaper32 or wallpaper64 in the same install), so compare install",
		"# directories instead of exact executables.",
		"$expectedDir = [IO.Path]::GetDirectoryName(\"" + executable.replace(/"/g, "\"\"") + "\")",
		"$actualDir = [IO.Path]::GetDirectoryName($actualExe)",
		"if ([string]::IsNullOrWhiteSpace($expectedDir) -or -not [string]::Equals($expectedDir, $actualDir, [StringComparison]::OrdinalIgnoreCase)) { throw \"DSH_DESKTOP_WINDOW_PROCESS_MISMATCH\" }",
...
app.whenReady().then(async () => {
	...
	runtime = new WallpaperEngineRuntime({
		...
		parkWindow: async ({ sourceId, title, executable }) => {
			try {
				const ack = await runDesktopNativeScript(windowParkScript({
					sourceId,
					title,
					executable
				}));
