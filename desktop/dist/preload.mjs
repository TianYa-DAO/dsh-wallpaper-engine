...
contextBridge.exposeInMainWorld("desktopWindow", {
	...
	setWallpaperMode: (payload) => invoke("dsh-desktop-wallpaper-set-enabled", payload ?? {}),
	updateWallpaperMode: (payload) => invoke("dsh-desktop-wallpaper-update", payload ?? {}),
	getDesktopModeStatus: () => invoke("dsh-desktop-mode-get-status"),
	setDesktopMode: (payload) => invoke("dsh-desktop-mode-set-enabled", payload ?? {}),
	setDesktopIconsVisible: (visible) => invoke("dsh-desktop-mode-set-icons-visible", visible !== false),
	probeDesktopIcons: () => invoke("dsh-desktop-mode-probe-icons"),
	setDesktopSoftwareLocked: (locked) => invoke("dsh-desktop-mode-set-software-lock", locked === true),
	requestDesktopKeyboardFocus: () => invoke("dsh-desktop-mode-request-keyboard-focus"),
	updateDesktopPointerRoute: (payload) => ipcRenderer.send("dsh-desktop-mode-pointer-route", payload ?? {}),
	onWallpaperModeState: (callback) => {
		if (typeof callback !== "function") return () => {};
		const listener = (_event, payload) => callback(payload ?? {});
		ipcRenderer.on("dsh-desktop-mode-state", listener);
		return () => {
			ipcRenderer.removeListener("dsh-desktop-mode-state", listener);
		};
	},
	...
	onDshInstallProgress: (callback) => {
		if (typeof callback !== "function") return () => {};
		const listener = (_event, payload) => callback(payload ?? {});
		ipcRenderer.on("dsh-install-progress", listener);
		return () => {
			ipcRenderer.removeListener("dsh-install-progress", listener);
		};
	},
