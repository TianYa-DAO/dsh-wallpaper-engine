...
rem Resolve repository root (apps/desktop/scripts -> ../../../).
cd /d "%~dp0..\..\.."

echo ============================================================
echo   DeepSeek Harness Desktop
...
start "dsh-web" cmd /c "cd /d ""%~dp0..\..\.."" && call pnpm dsh web"
...
call pnpm --filter @deepseek-ai/dsh-desktop run start
if errorlevel 1 (
  echo.
  echo [error] The desktop shell exited with an error.
  echo         Build it first: pnpm --filter @deepseek-ai/dsh-desktop run build
  pause
  exit /b 1
)
exit /b 0
